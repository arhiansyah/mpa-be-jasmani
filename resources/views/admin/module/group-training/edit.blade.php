            ...
            <div class="col-12">
                <div class="card">
                    ...
                    <div class="card-body p-3 p-md-4">
                        <form action="{{ route('group-training.update', $groupTraining->id) }}" method="post" enctype='multipart/form-data'>
                            @csrf
                            @method('put')
                            <table class="table table-responsive table-borderless">
                                <tbody>
                                    <tr>
                                        ...
                                        <td>:</td>
                                        <td>
                                            <input type="text" class="form-control @error('name') is-invalid @enderror" name="name" value="{{ old('name', $groupTraining->name) }}">
                                            @error('name')
                                    ...
                                    <tr>
                                        <td>Description</td>
                                        <td>:</td>
                                        <td><textarea name="description" id="description">{{ $groupTraining->description }}</textarea>
                                            @error('description')
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <select class="form-control trainingAll" name="training[]" multiple="multiple" style="width: 100%; height: 100%;">
                                                @foreach ($training as $key => $tra)
                                                @if(isset($groupTraining->training[$key]))
                                                <option value="{{ $tra->id }}" {{ $groupTraining->training[$key]->id == $tra->id ? 'selected' : '' }}>{{ $tra->name }}</option>
                                                @else
                                                <!-- break; -->
